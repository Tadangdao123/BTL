...
function getItem( isPrev , goTo ) {
	...
	// in any other case.. 
		...
		if (!isPrev) { 
			...
			// if this is the second fact...
			if (item==2) {  

				// move all facts (from 2 onwards) inside the container				
				var ic = $("#itemsContainer");
				if ( ic.find('.i').length==1 ) {
...
function getItemSlideEffect( i, isPrev ) {
	...
	switch(i) {
		case -2:
			obj.animate({ opacity: "0.4", left: "-200%" }, 500);
			break;
		case -1:
			if (isPrev) { obj.css("left", "-200%") };
			obj.animate({ opacity: "0.4", left: "-92.5%" }, 500, function(){  } );
			break;

		// place the item on the screen
		case 0: 
			obj.show();
			obj.animate({ opacity: "1", left: "2.5%" }, 500, function(){ });
			break;
		...
		case 1:
			...
			obj.show();
			obj.animate({ opacity: "0.4", left: "97.5%" }, 500);
			break;			

		// place the item up two steps next
		case 2:
			obj.animate({ opacity: "0.4", left: "200%" }, 500);
			break;			
	}
