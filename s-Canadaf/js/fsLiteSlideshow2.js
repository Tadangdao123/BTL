...
function getNext() {
	...
	for (var i=2; i<=totalItems; i++) {
		is = $("#i" + i).position().top;
		if (ws < is) {
			$('html,body').animate({ scrollTop: is + 1 }, 'slow');
			GAevent('LiteSnippet', 'next', i); 
			return false;
		}
	}

	//alert( "scroll: " + $(window).scrollTop() + " - i2:" + $("#i2").position().top ) 
}
...
//	if ($(window).width() > $(window).height()) {
		...
		// make all facts clear
//		$(".i").css("opacity", 1);
//		$(".i_s").css("opacity", 1);

		// next button: hide it if we're beyond the last fact.. or show it  
		if ($(window).scrollTop() > $("#i" + (totalItems)).position().top) {
			$('#next').hide()
			$('#nextT').hide()
		} else {
			$('#next').show()
...
//	} else {
		...
		// make all facts opaque
		$(".i").css("opacity", 0.1);
		$(".i_s").css("opacity", 0.1);

		// make current fact seen
		var ws = $(window).scrollTop();
		var is = 0;
		for (var i=1; i<=totalItems; i++) {
			...
			// if the item position is MORE than window scroll.. show the (first) item
			if (is > ws-2) {
				ix.css("opacity", 1);
				$("#i_s" + i).css("opacity", 1);
				i = totalItems+1;
...
function pKeyDown(e) {
	...
	if (evt.keyCode == 27 && alertVisible) { 
		$(".alert").hide();
	}
}
